...
class DfcfSeasonReportTask(DfcfReportBaseTask):
    def __init__(self):
        super(DfcfSeasonReportTask, self).__init__()
        self.base_url = 'http://dcfm.eastmoney.com/em_mutisvcexpandinterface/api/js/get?type=YJBB21_YJBB&token={token}&st=latestnoticedate&sr=-1&p={pageNo}&ps={pageSize}&js=var%20{varName}={pages:(tp),data:%20(x),font:(font)}&filter=(securitytypecode%20in%20(%27058001001%27,%27058001002%27))(reportdate=^{enddate}^)&rt={rt}'
        self.db_table = 'stock_dfcf_season_report'
        self.order_column = 'latestnoticedate'
        self.date_columns = ['reportdate', 'latestnoticedate']
        self.pageSize = 100

    def get_report_date(self):
        return self.get_last_season_end_date()